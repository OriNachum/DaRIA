"""IRC Skill Client — connects OpenCode to the daria daemon via Unix socket.

This module provides:
- ``SkillClient``: async client library for use from Python code
- CLI entry point: ``python -m daria.daemon.skill.irc_client <subcommand> ...``
...
class SkillClient:
    """Async client that connects to the daria daemon Unix socket."""
...
def _sock_path_from_env() -> str:
    """Resolve socket path from DARIA_NICK env var."""
    nick = os.environ.get("DARIA_NICK", "")
    if not nick:
        print("ERROR: DARIA_NICK environment variable is not set", file=sys.stderr)
        sys.exit(1)
    runtime_dir = os.environ.get("XDG_RUNTIME_DIR", "/tmp")
    return os.path.join(runtime_dir, f"daria-{nick}.sock")
